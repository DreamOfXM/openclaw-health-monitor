...
class TestDataCollector(unittest.TestCase):
    ...
    def test_get_metrics_falls_back_when_runtime_health_is_zero(
        ...
        fake_legacy.get_system_metrics.return_value = {
            "cpu": 0,
            "mem_used": 0,
            "mem_total": 0,
        }
        ...
        with (
            ...
            mock.patch.object(
                self.collector, "_shared_state", return_value={"summary": {}}
            ),
    ...
    def test_runtime_context_prefers_active_binding(self):
        ...
        with mock.patch(
            "services.data_collector._legacy_dashboard", return_value=fake_legacy
        ):
    ...
    def test_get_environment_exposes_binding_audit(self):
        ...
        with (
            mock.patch.object(
                self.collector, "_load_runtime_context", return_value=context
            ),
            mock.patch.object(
                self.collector,
                "_shared_state_fresh",
    ...
    def test_get_environment_probes_live_health_when_runtime_snapshot_is_stale_false(
        self,
    ):
        fake_legacy = mock.Mock()
        fake_legacy.get_gateway_process_for_env.return_value = {"pid": 4321}
    ...
    def test_restart_environment_delegates_to_legacy_restart(self):
        fake_legacy = mock.Mock()
        fake_legacy.restart_active_openclaw_environment.return_value = (
            True,
            "重启成功",
            "123",
            "456",
    ...
    def test_get_tasks_prefers_shared_state_snapshot(self):
        ...
        with (
            mock.patch.object(self.collector, "_shared_state", return_value=payload),
    ...
    def test_summarize_session_entries_prefers_human_task_title_and_explainable_state(
        self,
    ):
        entries = [
            {
        ...
        summary = self.collector._summarize_session_entries(
            entries, pathlib.Path("/tmp/pm.jsonl")
        )
    ...
    def test_summarize_session_entries_marks_exec_as_processing(self):
        ...
        summary = self.collector._summarize_session_entries(
            entries, pathlib.Path("/tmp/dev.jsonl")
        )
    ...
    @mock.patch("services.data_collector._legacy_dashboard")
    def test_get_learnings_hides_internal_control_promotions_from_user_view(
        self, mock_legacy_dashboard
    ):
        fake_legacy = mock.Mock()
    ...
    def test_fetch_environment_data_includes_version_and_recovery_profile(self):
        with (
            mock.patch.object(self.collector, "_load_runtime_context") as context_mock,
            mock.patch.object(
                self.collector,
                "_shared_state_fresh",
                return_value={
            ...
            mock.patch.object(
                ...
                side_effect=lambda name, default=None: {
                    ...
                    "openclaw-recovery-profile.json": {
                        "known_good": {"describe": "v2026.3.11"},
                        "rollback_hint": {"target_describe": "v2026.3.11"},
                    },
                    "watchdog-recovery-status.json": {},
                    "watchdog-recovery-incidents.json": [],
                    "watchdog-recovery-hints.json": [],
        ...
        self.assertEqual(
            data["recovery_profile"]["known_good"]["describe"], "v2026.3.11"
        )
    ...
    def test_emergency_recover_returns_known_good_guidance_when_no_snapshot_exists(
        self,
    ):
        fake_legacy = mock.Mock()
        ...
        fake_legacy.load_versions.return_value = {
            "known_good": {"describe": "v2026.3.11", "commit": "abc"}
        }
        with mock.patch(
            "services.data_collector._legacy_dashboard", return_value=fake_legacy
        ):
    ...
    def test_get_health_score_data_flags_main_closure_purity_gate(self):
        with (
            mock.patch.object(
                self.collector,
                "_fetch_environment_data",
                return_value={"gateway_healthy": True},
            ),
            mock.patch.object(
                self.collector,
                "_shared_state",
            ...
            mock.patch.object(
                self.collector, "get_metrics", return_value={"cpu_percent": 1.0}
            ),
        ):
    ...
    def test_get_tasks_normalizes_current_task_facts_shape(self):
        ...
        with (
            ...
            mock.patch(
                "services.data_collector._read_json_file", return_value=current_facts
            ),
            mock.patch.object(
                self.collector,
                "_load_runtime_context",
                return_value={"legacy": mock.Mock(), "selected_env": {"id": "primary"}},
            ),
        ):
        ...
        self.assertEqual(
            payload["current"]["root_task"]["root_task_id"], "legacy-root:task-1"
        )
        self.assertEqual(
            payload["current"]["root_task"]["workflow_state"], "delivery_pending"
        )
        self.assertEqual(
            payload["current"]["root_task"]["delivery_state"], "delivery_confirmed"
        )
        self.assertEqual(
            payload["current"]["current_workflow_run"]["workflow_run_id"],
            "legacy-run:task-1",
        )
        self.assertEqual(
            payload["current"]["current_finalizer"]["decision_state"], "finalized"
        )
        self.assertEqual(
            payload["current"]["current_delivery_attempt"]["current_state"],
            "delivery_confirmed",
        )
        self.assertEqual(payload["current"]["followup_count"], 1)
        self.assertEqual(
            payload["current"]["current_followups"][0]["followup_type"],
            "delivery_retry",
        )
