...
Health Monitor 可以托管一套“官方最新版 OpenClaw”的隔离验证环境，不需要直接改你当前在用的 OpenClaw 仓库。
验证环境和主用环境是可切换关系，不是双活关系。
...
- 自动生成独立的 gateway token，并关闭官方验证环境的 Control UI device auth，避免和主环境共用浏览器签名状态
- 在隔离目录中安装依赖并构建
- 用独立端口拉起验证 Gateway

这意味着：

- 切到官方验证版时，不需要你手工再改 `openclaw.json`
- 其他用户安装守护助手后，也不需要额外处理 `device signature invalid`
- 同一时间只有一个 Gateway 处于激活状态，避免同一通道被两套环境同时消费
...
Health Monitor can manage an isolated validation environment for the latest upstream OpenClaw without replacing your current working environment.
The primary environment and the validation environment are switchable, not active-active.
...
- creates a dedicated `origin/main` worktree
- syncs an isolated config and workspace
- generates an independent gateway token
- prepares a separate Control UI auth state
- installs dependencies and builds inside the isolated directory

This means:

- you do not need to edit the validation `openclaw.json` by hand
- only one Gateway should remain active at a time
...
For the official validation environment:
